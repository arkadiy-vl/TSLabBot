...
namespace BotDonchianPosAdd
{
    public static class TradeHelper
    {
        ...
        /// </summary>
        /// <param name="list1">Первая кривая</param>
        /// <param name="list2">Вторая кривая</param>
        /// <param name="numBar">Номер бара, на котором искать пересечение</param>
        /// <returns></returns>
        public static bool CrossUp(this IList<double> list1, IList<double> list2, int numBar)
        {
            return (list1[numBar - 1] <= list2[numBar - 1]) && (list1[numBar] > list2[numBar]);
        }
        ...
        /// <param name="numBar">Номер бара, на котором искать пересечение</param>
        /// <returns></returns>
        public static bool CrossDown(this IList<double> list1, IList<double> list2, int numBar)
        {
            return (list1[numBar - 1] >= list2[numBar - 1]) && (list1[numBar] < list2[numBar]);
        }

        /// <summary>
        /// Возвращает истину, если на интервале в два последних бара первая кривая пересекает вторую вверх.
        /// </summary>
        /// <param name="list1">Первая кривая</param>
        /// <param name="list2">Вторая кривая</param>
        /// <param name="numBar">Номер последнего бара диапазона, на котором искать пересечение</param>
        /// <returns></returns>
        public static bool CrossUpRange2(this IList<double> list1, IList<double> list2, int numBar)
        {
            return (list1[numBar - 2] <= list2[numBar - 2]) && (list1[numBar] > list2[numBar]);
        }

        /// <summary>
        /// Возвращает истину, если на интервале в два последних бара первая кривая пересекает вторую вниз.
        /// </summary>
        /// <param name="list1">Первая кривая</param>
        /// <param name="list2">Вторая кривая</param>
        /// <param name="numBar">Номер последнего бара диапазона, на котором искать пересечение</param>
        /// <returns></returns>
        public static bool CrossDownRange2(this IList<double> list1, IList<double> list2, int numBar)
        {
            return (list1[numBar - 2] >= list2[numBar - 2]) && (list1[numBar] < list2[numBar]);
        }
        ...
        /// <param name="numBar">Номер бара</param>
        /// <returns></returns>
        public static bool IsScalp(this IPosition pos, int numBar)
        {
            if (pos.IsActiveForBar(numBar))
        ...
        /// <summary>
        /// Получить среднюю цену входа для всех позиций из списка.
        /// </summary>
        /// <param name="positions">Список позиций</param>
        /// <returns>Средняя цена входа для списка позиций</returns>
        public static double GetAvgEntryPrice(this IList<IPosition> positions)
        {
            if (positions == null) return 0;
            var totalPrice = positions.Sum(p => p.GetPositionEntryValue());
            var totalSize = positions.Sum(p => p.GetPositionSize());
